...
int BACKEND_NODES=0;
int BACKEND_DEVICES=0;
char BACKEND_LIST[MAX_BACKEND_NODES+1][HOSTNAME_STRLEN];
int DEV_TO_INDEX[MAX_BACKEND_DEVICES];
int DEV_TO_LOCALDEV[MAX_BACKEND_DEVICES];

int DONE;
int first_node;
extern pthread_key_t tlsKey;
pthread_key_t tlsKey2 = 1;
...
inline int l_getLocalFromConfig(void){
    ...
    while(1)
    {
        ...
        if(s!=NULL)
		{
			strcpy(BACKEND_LIST[BACKEND_NODES], s); 
			if(strcmp(s, "localhost")==0)
				first_node = BACKEND_NODES;
			BACKEND_NODES++; 
			index++;
		}
        else
    ...
    fprintf(stderr, "\ncompleted all nodes but sched node\n"); 
    sprintf(key, "%s", "scheduler:sched", index);
    s = iniparser_getstring(d, key, NULL);
    strcpy(BACKEND_LIST[MAX_BACKEND_NODES], s);

    /*index = 0;
    while(1)
    {
	    sprintf(key, "%s%d\0", "gpu_weight:device", index);
	    s = iniparser_getstring(d, key, NULL);
	    if(s!=NULL)
	    {
		    gpu_weight[index++] = atoi(s);
	    }
	    else
	    {break;}
...
/* Finding the size of the rpc packet for the cubin version 4.0, with the new binary format
**/

int l_getFatRecPktSize_4(const __cudaFatCudaBinary2 *pFatCubin, cache_num_entries_t * pEntriesCache){
	...
	size += sizeof(pFatCubin->version);
        ...
        //now the header
        const __cudaFatCudaBinary2Header * header = (const __cudaFatCudaBinary2Header *)(pFatCubin->fatbinData);
        size += sizeof(header->magic);
        size += sizeof(header->version);
        size += sizeof(header->length);

        //now the binary
        const char * base = (const char *)(header + 1);
        unsigned long long int binary_len = header->length;
        size += binary_len;
	// now deal with strings;strlen() doesn't include NULL terminator
	// we will store those characters as the size as returned by strlen
	// then the string terminated plus NULL included (it will make simpler
	...
	//size of string 
	size += l_getStringPktSize(pFatCubin->f);
...
int getFatRecPktSize_4(const __cudaFatCudaBinary *pFatCubin, cache_num_entries_t * pEntriesCache){
	int size = 0;
	// clear cache - this is again cleaned in particular l_getSize__cudaFat...
	// functions
	pEntriesCache->ncubs = 0;
	...
	size = l_getFatRecPktSize_4(pFatCubin, pEntriesCache);
	//size += l_getSize__cudaFatBinaryEntry(pFatCubin->dependends, pEntriesCache);
...
int packFatBinary_4(char * pFatPack, __cudaFatCudaBinary2 * const pSrcFatC,
		cache_num_entries_t * const pEntriesCache){
	// to enabling counting the offset
	char * pFatPackOrig = pFatPack;
	int offset = 0;
	...
	// pack everything apart from dependends
	offset = l_packFatBinary_4(pFatPack, pSrcFatC, pEntriesCache);
	if ( ERROR == offset ) return ERROR; else pFatPack += offset;
...
int l_packEntries_4(char * pFatPack, __cudaFatCudaBinary2Entry * entry)
{
	memcpy(pFatPack, (unsigned int *)&entry->type, sizeof(entry->type));
	pFatPack += sizeof(entry->type);
	memcpy(pFatPack, (unsigned int*)&entry->binary, sizeof(entry->binary));
	pFatPack += sizeof(entry->binary);
	memcpy(pFatPack, (unsigned int*)&entry->binarySize, sizeof(entry->binarySize));
	pFatPack += sizeof(entry->binarySize);
	memcpy(pFatPack, (unsigned int*)&entry->unknown2, sizeof(entry->unknown2));
	pFatPack += sizeof(entry->unknown2);
	memcpy(pFatPack, (unsigned int*)&entry->kindOffset, sizeof(entry->kindOffset));
	pFatPack += sizeof(entry->kindOffset);
	memcpy(pFatPack, (unsigned int*)&entry->unknown3, sizeof(entry->unknown3));
	pFatPack += sizeof(entry->unknown3);
	memcpy(pFatPack, (unsigned int*)&entry->unknown4, sizeof(entry->unknown4));
	pFatPack += sizeof(entry->unknown4);
	memcpy(pFatPack, (unsigned int*)&entry->unknown5, sizeof(entry->unknown5));
	pFatPack += sizeof(entry->unknown5);
	memcpy(pFatPack, (unsigned int*)&entry->name, sizeof(entry->name));
	pFatPack += sizeof(entry->name);
	memcpy(pFatPack, (unsigned int*)&entry->nameSize, sizeof(entry->nameSize));
	pFatPack += sizeof(entry->nameSize);
	memcpy(pFatPack, (unsigned long long int*)&entry->unknown6, sizeof(entry->unknown6));
	pFatPack += sizeof(entry->unknown6);
	memcpy(pFatPack, (unsigned long long int*)&entry->unknown7, sizeof(entry->unknown7));
	pFatPack += sizeof(entry->unknown7);
}

int l_packFatBinary_4(char * pFatPack, __cudaFatCudaBinary2 * const pSrcFatC,
		cache_num_entries_t * const pEntriesCache){
	...
	memcpy(pFatPack, (int *)&pSrcFatC->magic, sizeof(pSrcFatC->magic));
	pFatPack += sizeof(pSrcFatC->magic);
	memcpy(pFatPack, (int *)&pSrcFatC->version, sizeof(pSrcFatC->version));
	pFatPack += sizeof(pSrcFatC->version);

	//now the header
	const __cudaFatCudaBinary2Header * header = (const __cudaFatCudaBinary2Header *)(pSrcFatC->fatbinData);
	memcpy(pFatPack, (unsigned int *)&header->magic, sizeof(header->magic));
	pFatPack += sizeof(header->magic);
	memcpy(pFatPack, (unsigned int *)&header->version, sizeof(header->version));
	pFatPack += sizeof(header->version);
	memcpy(pFatPack, (unsigned long long int *)&header->length, sizeof(header->length));
	pFatPack += sizeof(header->length);
	unsigned long long int * read_pointer = pFatPack - sizeof(unsigned long long int);
	printf("\nBinary length:%x\n", *read_pointer);

	//now the binary
        const char * base = (const char *)(header + 1);
	unsigned long long int binary_len = header->length;
	memcpy(pFatPack, (const char *)base, binary_len); 
	pFatPack += binary_len;

	// now the string
	offset = l_packStr(pFatPack, pSrcFatC->f);
	if ( ERROR == offset ) return ERROR; else pFatPack += offset;

	fprintf(stderr, "packed the binary\n");   
...
int unpackFatBinary_4(__cudaFatCudaBinary2 *pFatC, char * pFatPack){
	// to remember how many bytes we have read
	char * pFatPackOrig = pFatPack;
	
	// how many bytes we have read
	int offset;
	printf("\nInitial magic pointer:%x\n", *(int *)pFatPack);
	memcpy(&pFatC->magic, (int *)pFatPack, sizeof(pFatC->magic));
	pFatPack += sizeof(pFatC->magic);
	memcpy(&pFatC->version, (int *) pFatPack, sizeof(pFatC->version));
	pFatPack += sizeof(pFatC->version);
   
        const __cudaFatCudaBinary2Header * header_base = (const __cudaFatCudaBinary2Header *)(pFatPack); 
	pFatC->fatbinData = malloc(sizeof(__cudaFatCudaBinary2Header) + header_base->length);

	fprintf(stderr, "start unpacking header\n");
	//now unpacking header
	__cudaFatCudaBinary2Header * header = (__cudaFatCudaBinary2Header *)(pFatC->fatbinData);
	memcpy(&header->magic, (unsigned int *)pFatPack, sizeof(header->magic));
	pFatPack += sizeof(header->magic);
	memcpy(&header->version, (unsigned int *)pFatPack, sizeof(header->version));
	pFatPack += sizeof(header->version);
	memcpy(&header->length, (unsigned long long int *)pFatPack, sizeof(header->length));
	pFatPack += sizeof(header->length);
	unsigned long long int * read_pointer = pFatPack - sizeof(unsigned long long int);
	printf("\nBinary length:%x\n", *read_pointer);

	fprintf(stderr, "start unpacking binary\n");
	//now the binary
        const char * base = (const char *)(header + 1);
	unsigned long long int binary_len = header->length;
	memcpy((char *)base, (const char *)pFatPack, binary_len); 
	pFatPack += binary_len;

	// now unpack strings
	pFatC->f = l_unpackStr(pFatPack, &offset); 
	if ( ERROR == offset ) return ERROR; else pFatPack += offset;
